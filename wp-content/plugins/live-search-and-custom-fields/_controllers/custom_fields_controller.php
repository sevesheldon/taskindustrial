	...
	public function display_custom_fields_meta_box() {
		...
		if ( 'post.php' === $pagenow || 'post-new.php' === $pagenow ) {
			...
			if ( isset( $post_type ) && in_array( $post_type, $active_posts_type, true ) ) {
				...
				add_meta_box( 'lscf-custom-fields', 'LSCF Custom Fields', array( $this, 'display_custom_fields' ), $post_type, 'normal', 'high' );
	...
	public static function save_custom_fields() {
		...
		if ( isset( $_POST['save_px_post_fields'] ) ) {
			...
			foreach ( $_POST as $key => $value ) :
				...
				if ( preg_match( '/([a-zA-Z0-9_]+)__(pxid_[0-9a-z]+_[0-9]{1,3})_-_px-opt_(.+)$/i', $key, $matches ) ) {
				...
				if ( preg_match( '/([a-zA-Z0-9_]+)__(pxid_[0-9a-z]+_[0-9]{1,3})___([a-z_]+?)_-_name$/i', $key, $matches ) ) {
					...
					if ( 'px_icon_check_box' != $matches[3] ) {
						...
						if ( ! is_array( $_POST[ $value_key ] ) ) {
							...
							$fields[ $id ]['value'] = sanitize_text_field( wp_unslash( $_POST[ $value_key ] ) );
					...
					$fields[ $id ]['name'] = $value; // already sanitized on top.
