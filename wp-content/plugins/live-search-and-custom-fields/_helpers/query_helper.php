...
class PxWpQuery extends WP_Query {
	...
	public function px_cap_search_fields( $where ) {
		...
		foreach ( $custom_fields as $custom_field ) {
			...
			if ( 'range' == $type || 'date-interval' == $type || 'checkbox_post_terms' == $type || 'main-search' == $type ) {
				continue;
			}

			$id = esc_sql( $custom_field->ID );
			...
			if ( ! is_array( $custom_field->value ) ) {
				...
				$value = sanitize_text_field( wp_slash( $custom_field->value ) );
				$regex_value = lscf_sql_regex_escape( $value );
				...
				foreach ( $value as $val ) {
					...
					if ( ( count( $value ) - 1 ) == $c ) {
						$regex_val .= lscf_sql_regex_escape( sanitize_text_field( wp_slash( $val ) ) );
					} else {
						$regex_val .= lscf_sql_regex_escape( sanitize_text_field( wp_slash( $val ) ) ) . '|';
					}
