...
if(!function_exists('edgt_admin_scripts_init')) {
	...
	function edgt_admin_scripts_init() {
         ...
         */
        do_action('edgt_admin_scripts_init');
	}
...
if(!function_exists('edgt_enqueue_admin_styles')) {
	...
	function edgt_enqueue_admin_styles() {
         ...
         */
        do_action('edgt_enqueue_admin_styles');
	}
}
...
if(!function_exists('edgt_enqueue_admin_scripts')) {
	...
	function edgt_enqueue_admin_scripts() {
         ...
         */
        do_action('edgt_enqueue_admin_scripts');
	}
}
...
if(!function_exists('edgt_enqueue_meta_box_styles')) {
	...
	function edgt_enqueue_meta_box_styles() {
         ...
         * @see EdgeSkinAbstract::enqueueStyles - hooked with 10
         */
        do_action('edgt_enqueue_meta_box_styles');
	}
}
...
if(!function_exists('edgt_enqueue_meta_box_scripts')) {
	...
	function edgt_enqueue_meta_box_scripts() {
         ...
         * @see EdgeSkinAbstract::enqueueScripts - hooked with 10
         */
        do_action('edgt_enqueue_meta_box_scripts');
	}
}
...
if(!function_exists('edgtf_init_theme_options_array')) {
	...
	function edgtf_init_theme_options_array() {
		...
		if(is_array($db_options)) {
			...
			$edgt_options  = array_merge($edgtFramework->edgtOptions->options, get_option('edgt_options_hudson'));
		} else {
...
if(!function_exists('edgt_init_theme_options')) {
	/**
	 * Function that sets $edgt_options variable if it does'nt exists
	 */
	function edgt_init_theme_options() {
		global $edgt_options;
		global $edgtFramework;
		if(isset($edgt_options['reset_to_defaults'])) {
			if( $edgt_options['reset_to_defaults'] == 'yes' ) delete_option( "edgt_options_hudson");
		}
...
if(!function_exists('edgt_theme_menu')) {
	...
	function edgt_theme_menu() {
		global $edgtFramework;
		edgt_init_theme_options();

		$page_hook_suffix = add_menu_page(
			'Task Website Options',                   // The value used to populate the browser's title bar when the menu page is active
			'Task Website Options',                   // The text of the menu in the administrator's sidebar
			'administrator',                  // What roles are able to access the menu
			'edgt_theme_menu',                // The ID used to bind submenu items to this menu
			array($edgtFramework->getSkin(), 'renderOptions'), // The callback function used to render this menu
			$edgtFramework->getSkin()->getMenuIcon('options'),             // Icon For menu Item
			$edgtFramework->getSkin()->getMenuItemPosition('options')            // Position
		);
		...
		foreach ($edgtFramework->edgtOptions->adminPages as $key=>$value ) {
			...
			$subpage_hook_suffix = add_submenu_page(
				'edgt_theme_menu',
				'Task Website Options - '.$value->title,                   // The value used to populate the browser's title bar when the menu page is active
				$value->title,                   // The text of the menu in the administrator's sidebar
				'administrator',                  // What roles are able to access the menu
				'edgt_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
				array($edgtFramework->getSkin(), 'renderOptions')
			);
...
if(!function_exists('edgtf_save_options')) {
	...
	function edgtf_save_options() {
		...
		$edgt_options = array_merge($edgt_options, $_REQUEST);

		update_option( 'edgt_options_hudson', $edgt_options );
...
if(!function_exists('edgt_generate_icon_pack_options')) {
    /**
     * Generates options HTML for each icon in given icon pack
     * Hooked to wp_ajax_update_admin_nav_icon_options action
     */
    function edgt_generate_icon_pack_options() {
        global $edgtIconCollections;

        $html = '';
        $icon_pack = isset($_POST['icon_pack']) ? $_POST['icon_pack'] : '';
        $collections_object = $edgtIconCollections->getIconCollection($icon_pack);
    ...
    add_action('wp_ajax_update_admin_nav_icon_options', 'edgt_generate_icon_pack_options');
}
