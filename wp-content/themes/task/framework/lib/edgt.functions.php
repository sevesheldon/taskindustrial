...
if (!function_exists('edgt_generate_dynamic_css_and_js')){
	...
	function edgt_generate_dynamic_css_and_js() {
		...
		$edgt_options = get_option('edgt_options_hudson');
...
if(!function_exists('edgt_core_installed')) {
    /**
     * Checks if core plugin is installed
     * @return bool
     */
    function edgt_core_installed() {
        return defined('EDGE_CORE_VERSION');
    }
}

if(!function_exists('edgt_core_plugin_message')) {
    /**
     * Function that prints a mesasge in the admin if user hides TGMPA plugin activation message
     */
    function edgt_core_plugin_message() {
        if(get_user_meta(get_current_user_id(), 'tgmpa_dismissed_notice', true) && !edgt_core_installed()) {
            echo apply_filters('edgt_core_plugin_message', '<div class="update-nag">'.__('Installation of the "Edge
            Core" plugin is
            essential for
            proper
    ...
    add_action('admin_notices', 'edgt_core_plugin_message');
}
...
if(!function_exists('edgt_resize_image')) {
    /**
     ...
     */
    function edgt_resize_image($attach_id = null, $attach_url = null, $width = null, $height = null, $crop = true) {
        $return_array = array();

        //is attachment id empty?
        if(empty($attach_id) && $attach_url !== '') {
            //get attachment id from url
            $attach_id = edgt_get_attachment_id_from_url($attach_url);
        }
...
if(!function_exists('edgt_generate_thumbnail')) {
    /**
     * Generates thumbnail img tag. It calls edgt_resize_image function which resizes img on the fly
     ...
     *
     * @see edgt_resize_image()
     * @see edgt_get_attachment_id_from_url()
     */
    function edgt_generate_thumbnail($attach_id = null, $attach_url = null, $width = null, $height = null, $crop = true) {
        //is attachment id empty?
        if(empty($attach_id)) {
        ...
        if(!empty($attach_id) || !empty($attach_url)) {
            $img_info = edgt_resize_image($attach_id, $attach_url, $width, $height, $crop);
