...
include_once('includes/edgt-options-helper-functions.php');
...
include_once('includes/plugins/edgt-cpt.php');
include_once('includes/instagram/edgt-instagram-api.php');
include_once('includes/edgt-blog-functions.php');
include_once('includes/edgt-plugin-helper-functions.php');
include_once('widgets/edgt-call-to-action-widget.php');
include_once('widgets/edgt-sticky-sidebar.php');
include_once('widgets/edgt-latest-posts-widget.php');
include_once('widgets/edgt-latest-posts-menu-widget.php');
include_once('widgets/edgt-instagram-widget.php');
...
if(function_exists("is_woocommerce")){
	...
	include_once('widgets/edgt-woocommerce-dropdown-cart.php');
}
...
if (!function_exists('edgt_styles')) {
	...
	function edgt_styles() {
		...
		if(edgt_is_side_header() && $responsiveness != "no" && $vertical_area_hidden!='hidden'){
        ...
        if (isset($edgt_landing)) {
            ...
            wp_enqueue_style("edgt_landing_fancybox", get_home_url() . "/demo-files/landing/css/jquery.fancybox.css");
            wp_enqueue_style("edgt_landing", get_home_url() . "/demo-files/landing/css/landing_stylesheet_stripped.css");
		...
		if (isset($edgt_toolbar)) {
			...
			wp_enqueue_style("edgt_toolbar", get_home_url() . "/demo-files/toolbar/css/toolbar.css");
...
if(!function_exists('edgt_google_fonts_styles')) {
	...
	function edgt_google_fonts_styles() {
		...
		if(is_array($edgtFramework->edgtOptions->getOptionsByType('fontsimple')) && count($edgtFramework->edgtOptions->getOptionsByType('fontsimple')) > 0){
			$font_sipmle_field_array = $edgtFramework->edgtOptions->getOptionsByType('fontsimple');
		}
		$font_field_array = array();
		if(is_array($edgtFramework->edgtOptions->getOptionsByType('font')) && count($edgtFramework->edgtOptions->getOptionsByType('font')) > 0){
			$font_field_array = $edgtFramework->edgtOptions->getOptionsByType('font');
		}
		...
		while ( $loop->have_posts() ) : $loop->the_post();
			...
			if(get_post_meta(get_the_ID(), "edgt_slide-title-font-family", true) != "" && !edgt_is_native_font(get_post_meta(get_the_ID(), "edgt_slide-title-font-family", true))) {
			...
			if(get_post_meta(get_the_ID(), "edgt_slide-text-font-family", true) != "" && !edgt_is_native_font(get_post_meta(get_the_ID(), "edgt_slide-text-font-family", true))) {
			...
			if(get_post_meta(get_the_ID(), "edgt_slide-subtitle-font-family", true) != "" && !edgt_is_native_font(get_post_meta(get_the_ID(), "edgt_slide-subtitle-font-family", true))) {
		...
		$default_font_string = 'Open+Sans:'.$font_weight_str.'%7CMerriweather:'.$font_weight_str.'%7CRaleway:'.$font_weight_str.'%7CPlayfair+Display:'.$font_weight_str.'%7CLato:'.$font_weight_str;
...
if (!function_exists('edgt_scripts')) {
	...
	function edgt_scripts() {
	   ...
	   //include google map api script
		if ($edgt_options['google_maps_api_key'] != '' && isset($edgt_options['google_maps_api_key']) ) {
			$google_maps_api_key = $edgt_options['google_maps_api_key'];
        ...
        //is landing enabled?
        if(isset($edgt_toolbar)) {
            wp_enqueue_script("edgt_toolbar", get_home_url() . "/demo-files/toolbar/js/toolbar_default.js",array(),false,true);
        }
	...
	function edgt_woocommerce_assets() {
		...
		if(edgt_is_woocommerce_installed()) {
			if(edgt_load_woo_assets()) {
				...
				wp_enqueue_script("edgt_woocommerce_script", EDGE_ROOT . "/js/woocommerce.min.js", array(), false, true);
...
if (!function_exists('edgt_register_menus')) {
	...
	function edgt_register_menus() {
        ...
        if((isset($edgt_options['header_bottom_appearance']) && $edgt_options['header_bottom_appearance'] != "stick_with_left_right_menu") || (edgt_is_side_header())){
...
if (!function_exists('edgt_ajax_classes')) {
	...
	function edgt_ajax_classes($classes) {
		...
		if(($edgt_options['page_transitions'] === "0")) {
			$classes[] = '';
		}

		//is up down animation type set?
		elseif($edgt_options['page_transitions'] === "1") {
			$classes[] = 'ajax_updown';
			$classes[] = 'page_not_loaded';
		}

		//is fade animation type set?
		elseif($edgt_options['page_transitions'] === "2") {
			$classes[] = 'ajax_fade';
			$classes[] = 'page_not_loaded';
		}

		//is up down fade animation type set?
		elseif($edgt_options['page_transitions'] === "3") {
			$classes[] = 'ajax_updown_fade';
			$classes[] = 'page_not_loaded';
		}

		//is left / right animation type set?
		elseif($edgt_options['page_transitions'] === "4") {
...
if (!function_exists('edgt_vertical_menu_class')) {
	...
	function edgt_vertical_menu_class($classes) {
		...
		if(edgt_is_side_header()) {
			...
			if(isset($edgt_options['vertical_area_position'])){
				if($edgt_options['vertical_area_position'] == 'right'){
					$classes[] = ' vertical_menu_right';
				}elseif($edgt_options['vertical_area_position'] == 'left'){
		...
		if(isset($edgt_options['vertical_area_transparency_over_slider']) && $edgt_options['vertical_area_transparency_over_slider'] =='yes' && get_post_meta($id, "edgt_page_vertical_area_transparency_over_slider", true) != "no" && isset($edgt_options['vertical_area_dropdown_showing']) && $edgt_options['vertical_area_dropdown_showing'] != "side"){
			$classes[] = 'vertical_menu_background_opacity_over_slider  vertical_menu_background_opacity_over_slider_on';
		}else if(get_post_meta($id, "edgt_page_vertical_area_transparency_over_slider", true) == "yes" && isset($edgt_options['vertical_area_dropdown_showing']) && $edgt_options['vertical_area_dropdown_showing'] != "side"){
			$classes[] = ' vertical_menu_background_opacity_over_slider  vertical_menu_background_opacity_over_slider_on';
		}
...
if (!function_exists('edgt_ribbon_menu_class')) {
	/**
	 * Function that adds classes on body element for ribbon menu area
	 */
	function edgt_ribbon_menu_class($classes) {
		global $edgt_options;
		global $wp_query;

		//is left menu area turned on?
		if(edgt_is_ribbon_header()) {
			$classes[] = 'ribbon_menu_enabled';	
		}
		
		return $classes;
	}

	add_filter('body_class', 'edgt_ribbon_menu_class');
}
...
if(!function_exists('edgt_ajax_meta')) {
	...
	function edgt_ajax_meta() {
		...
		$seo_description = get_post_meta(edgt_get_page_id(), "edgt_seo_description", true);
		$seo_keywords = get_post_meta(edgt_get_page_id(), "edgt_seo_keywords", true);
		?>
...
if(!function_exists('edgt_user_scalable_meta')) {
    /**
     * Function that outputs user scalable meta if responsiveness is turned on
     * Hooked to edgt_header_meta action
     */
    function edgt_user_scalable_meta() {
        //is responsiveness option is chosen?
        if (edgt_is_responsive_on()) { ?>
    ...
    add_action('edgt_header_meta', 'edgt_user_scalable_meta');
}
...
if(!function_exists('edgt_side_menu_body_class')) {
    ...
    function edgt_side_menu_body_class($classes) {
			...
			if(isset($edgt_options['enable_side_area']) && $edgt_options['enable_side_area'] == 'yes') {
				   ...
				   if(isset($edgt_options['side_area_type']) && $edgt_options['side_area_type'] == 'side_area_uncovered_from_content') {
    ...
    */
    function edgt_paspartu_body_class($classes) {
        ...
        if(isset($edgt_options['paspartu']) && $edgt_options['paspartu'] == 'yes') {
			...
			if(($edgt_options['paspartu_on_bottom'] == 'yes' && $edgt_options['paspartu_on_bottom_fixed'] == 'yes') ||
			(edgt_is_side_header() && isset($edgt_options['vertical_menu_inside_paspartu']) && $edgt_options['vertical_menu_inside_paspartu'] == 'yes')) {
				$classes[] = 'paspartu_on_bottom_fixed';
			}

			if(isset($edgt_options['paspartu_header_alignment']) && $edgt_options['paspartu_header_alignment'] == 'yes'){
				$classes[] = 'paspartu_header_aligned';
			}

			if(isset($edgt_options['paspartu_header_inside']) && $edgt_options['paspartu_header_inside'] == 'yes'){
				$classes[] = 'paspartu_header_set_inside';
			}
			
			if(edgt_is_side_header() && isset($edgt_options['vertical_menu_inside_paspartu']) && $edgt_options['vertical_menu_inside_paspartu'] == 'no') {
				$classes[] = 'vertical_menu_outside_paspartu';
			}
			
			if(edgt_is_side_header() && isset($edgt_options['vertical_menu_inside_paspartu']) && $edgt_options['vertical_menu_inside_paspartu'] == 'yes') {
				$classes[] = 'vertical_menu_inside_paspartu';
			}
			
			if($edgt_options['paspartu_border_on_edges'] == 'yes' &&
				$edgt_options['paspartu_on_top'] == 'yes' && $edgt_options['paspartu_on_top_fixed'] == 'yes' && 
				$edgt_options['paspartu_on_bottom'] == 'yes' && $edgt_options['paspartu_on_bottom_fixed'] == 'yes' ) {
...
if(!function_exists('edgt_overlapping_content_body_class')) {
    ...
    function edgt_overlapping_content_body_class($classes) {
        ...
        if(isset($edgt_options['overlapping_content']) && $edgt_options['overlapping_content'] == 'yes') {
            ...
            if(isset($edgt_options['animate_overlapping_content']) && $edgt_options['animate_overlapping_content'] == 'yes') {
                $classes[] = 'animate_overlapping_content';
            }

            if(isset($edgt_options['frame_around_overlapping_content']) && $edgt_options['frame_around_overlapping_content'] == 'yes') {
...
if(!function_exists('edgt_set_woocommerce_body_class')) {
    ...
    add_filter('body_class', 'edgt_set_woocommerce_body_class');
}

if(!function_exists('edgt_set_blog_body_class')) {
    /**
     ...
     */
    function edgt_set_blog_body_class($classes) {		
    ...
    add_filter('body_class', 'edgt_set_blog_body_class');
}
...
if(!function_exists('edgt_full_screen_sections_on_small_screens_class')) {
    /**
     ...
     */
    function edgt_full_screen_sections_on_small_screens_class($classes) {
        global $edgt_options;
        
		if(get_post_meta(edgt_get_page_id(), "edgt_full_screen_sections_on_small_screens", true) !== ''){
			if(get_post_meta(edgt_get_page_id(), "edgt_full_screen_sections_on_small_screens", true) == 'yes'){
				$classes[] = 'enable_full_screen_sections_on_small_screens';
			}
		}else if(isset($edgt_options['full_screen_sections_on_small_screens']) && $edgt_options['full_screen_sections_on_small_screens'] == 'yes') {
			$classes[] = 'enable_full_screen_sections_on_small_screens';
		}        
    ...
    add_filter('body_class', 'edgt_full_screen_sections_on_small_screens_class');
}
...
if(!function_exists('edgt_set_logo_sizes')) {
	...
	function edgt_set_logo_sizes() {
		...
		if (isset($edgt_options['ribbon_closed_image'])){
			//get logo image size
			$ribbon_closed_logo_image_sizes = edgt_get_image_dimensions($edgt_options['ribbon_closed_image']);
			$edgt_options['ribbon_closed_image_width'] = 184;
			$edgt_options['ribbon_closed_image_height'] = 184;
	
			//is image width and height set?
			if(isset($logo_image_sizes['width']) && isset($logo_image_sizes['height'])) {
				//set those variables in global array
				$edgt_options['ribbon_closed_image_width'] = $ribbon_closed_logo_image_sizes['width'];
				$edgt_options['ribbon_closed_image_height'] = $ribbon_closed_logo_image_sizes['height'];
			}
		}
	}
