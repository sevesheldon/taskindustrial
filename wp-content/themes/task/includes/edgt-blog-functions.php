...
if(!function_exists('edgt_read_more_button')) {
	...
	function edgt_read_more_button($option = '', $class = '') {
        ...
        if(strpos($class, 'with_icon') !== false) {
            if (isset($edgt_options[$option.'_icon']) && $edgt_options[$option.'_icon'] == 'yes' && isset($edgt_options[$option.'_icon_pack']) && !empty($edgt_options[$option.'_icon_pack'])) {

                $icon_collection_obj = $edgtIconCollections->getIconCollection($edgt_options[$option.'_icon_pack']);
                if(isset($edgt_options[$option.'_icon_'.$icon_collection_obj->param]) && !empty($edgt_options[$option.'_icon_'.$icon_collection_obj->param])){
                    if (method_exists($icon_collection_obj, 'render')) {
                        $show_more_icon .= $icon_collection_obj->render($edgt_options[$option.'_icon_'.$icon_collection_obj->param], array(
        ...
        if($show_read_more_button == 'yes' && !edgt_post_has_read_more() && !post_password_required()) {
			echo apply_filters(
				...
				'<a href="'.get_the_permalink().'" target="_self" class="qbutton small read_more_button '. $class .' '. $animation_class .'"><span class="text_wrap">'.__("Read More", "edgt").'</span>'.$show_more_icon.$span.'</a>',
				$option,
				$class
			);
		}
...
if(!function_exists('edgt_excerpt')) {
	...
	function edgt_excerpt() {
		global $edgt_options, $edgt_word_count, $post;
		...
		elseif($edgt_word_count != '0') {
			//if word count is set and different than empty take that value, else that general option from theme options
			$word_count = isset($edgt_word_count) && $edgt_word_count !== "" ? $edgt_word_count : esc_attr($edgt_options['number_of_chars']);
...
if(!function_exists('edgt_set_blog_word_count')) {
	...
	function edgt_set_blog_word_count($word_count_param) {
		global $edgt_word_count;

		$edgt_word_count = $word_count_param;
	}
}
