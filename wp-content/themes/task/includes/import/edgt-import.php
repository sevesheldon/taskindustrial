...
class Edge_Import {
    ...
    public function import_content($file){
        ...
        if(is_wp_error($returned_value)){
            $this->message = esc_html__("An Error Occurred During Import", "edgt");
        }
        else {
            $this->message = esc_html__("Content imported successfully", "edgt");
        }
        ob_get_clean();
    }
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'edgt_options_hudson', $options);
    ...
    public function file_options($file){
        ...
        /*if ( file_exists($file_for_import) ) {
            ...
            $this->message = __("File doesn't exist", "edgt");
        }*/
    ...
    function edgt_admin_import() {
		...
		$this->pagehook = add_submenu_page(
			...
			array($edgtFramework->getSkin(), 'renderImport')
		);

        add_action('admin_print_scripts-'.$this->pagehook, 'edgt_enqueue_admin_scripts');
        add_action('admin_print_styles-'.$this->pagehook, 'edgt_enqueue_admin_styles');
...
global $edgt_import_object;
$edgt_import_object = new Edge_Import();
...
if(!function_exists('edgt_dataImport')){
    function edgt_dataImport(){
        ...
        if ($_POST['import_attachments'] == 1)
            $edgt_import_object->attachments = true;
        else
            $edgt_import_object->attachments = false;

        $folder = "hudson1/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $edgt_import_object->import_content($folder.$_POST['xml']);
...
if(!function_exists('edgt_widgetsImport')){
    function edgt_widgetsImport(){
        ...
        $edgt_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
...
if(!function_exists('edgt_optionsImport')){
    function edgt_optionsImport(){
        ...
        $edgt_import_object->import_options($folder.'options.txt');
...
if(!function_exists('edgt_otherImport')){
    function edgt_otherImport(){
        ...
        $edgt_import_object->import_options($folder.'options.txt');
        $edgt_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $edgt_import_object->import_menus($folder.'menus.txt');
        $edgt_import_object->import_settings_pages($folder.'settingpages.txt');
