...
if (!function_exists('edgt_pagination')) {
	function edgt_pagination($pages = '', $range = 4, $paged = 1){
		...
		$pagination_style = "";
		if ( isset($edgt_options['blog_pagination_border_above_yesno']) && $edgt_options['blog_pagination_border_above_yesno'] == 'yes' ){
			$pagination_classes .= " pagination_with_border";
			if ( isset($edgt_options['blog_pgn_border_color']) && $edgt_options['blog_pgn_border_color'] != '' ){
				$pagination_style .= "border-top-color:" . $edgt_options['blog_pgn_border_color'] . "; ";
			}
			else {
				$pagination_style .= "border-top-color: #dedede; ";
			}

			if ( isset($edgt_options['blog_pgn_border_width']) && $edgt_options['blog_pgn_border_width'] != '' ){
				$pagination_style .= "border-top-width:" . $edgt_options['blog_pgn_border_width'] . "px; ";
			}
			else {
				$pagination_style .= "border-top-width: 1px; ";
			}

			if ( isset($edgt_options['blog_pgn_border_style']) && $edgt_options['blog_pgn_border_style'] != '' ){
				$pagination_style .= "border-top-style:" . $edgt_options['blog_pgn_border_style'] . "; ";
			}
			else {
				$pagination_style .= "border-top-style: solid; ";
			}

			if ( isset($edgt_options['blog_pagination_border_margin']) && $edgt_options['blog_pagination_border_margin'] != '' ){
				$pagination_style .= "padding-top:" . $edgt_options['blog_pagination_border_margin'] . "px; ";
			}
			else {
		...
		if(1 != $pages){
			...
			echo "<div class='pagination ".esc_attr($pagination_classes)."' ".edgt_get_inline_style($pagination_style)."><ul>";
