...
$blog_show_small_separator = "no";
$blog_separator_alignment = "";
if (isset($edgt_options['blog_masonry_gallery_show_small_separator'])) {
	$blog_show_small_separator = $edgt_options['blog_masonry_gallery_show_small_separator'];
}
if (isset($edgt_options['blog_masonry_gallery_post_alignment']) && $edgt_options['blog_masonry_gallery_post_alignment'] !== ''){
	$blog_separator_alignment = " separator_align_".$edgt_options['blog_masonry_gallery_post_alignment'];
}

$blog_read_more_button_classes = '';
if (isset($edgt_options['blog_masonry_gallery_read_more_button_icon']) && $edgt_options['blog_masonry_gallery_read_more_button_icon'] == 'yes'){
...
<article id="post-<?php the_ID(); ?>" <?php post_class($blog_post_size); ?>>
		...
		<?php if ($_post_format == "quote" || $_post_format == "link"){ ?>
		<div class="post_text_pattern_holder">
		<?php } ?>
			<div class="post_text_inner">
				<div class="post_text_inner2">
					...
					<?php if ($blog_show_small_separator == "yes"){ ?>
					<span class="small separator <?php echo esc_attr($blog_separator_alignment);?>"></span>
					<?php } ?>
					...
					<?php } ?>
					<?php
                        edgt_read_more_button('blog_masonry_gallery_read_more_button',$blog_read_more_button_classes);
					?>
