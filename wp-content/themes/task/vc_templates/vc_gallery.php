...
if(version_compare(edgt_get_vc_version(), '4.7.4') >= 0) {
	...
	if ( 'nivo' === $type ) {
		...
		if ($type == "flexslider_slide" || $type == "flexslider_fade") {
			...
			if ($border_pattern == "yes" && $type == "flexslider_fade") {
				$li_classes .= 'border_pattern';

				if ($border_pattern_image != "") {
					if (is_numeric($border_pattern_image)) {
						$background_image_src = wp_get_attachment_url($border_pattern_image);
					} else {
						$background_image_src = $border_pattern_image;
					}
					$border_pattern_styles[] = "background-image: url('" . $background_image_src . "')";
				}

				if ($border_pattern_width != "") {
					$border_pattern_width = (strstr($border_pattern_width, 'px', true)) ? $border_pattern_width : $border_pattern_width . "px";
					$border_pattern_styles[] = "padding:" . $border_pattern_width;
				}
		...
		$el_start = '<li class="' . esc_attr($li_classes) . '" ' . edgt_get_inline_style($border_pattern_styles) . ' >';
		$el_end = '</li>';
...
//		$slides_wrap_end = '</ul>';
	...
	if(version_compare(edgt_get_vc_version(), '4.6.2') >= 0) {
		...
		extract(shortcode_atts(array(
			...
			'grayscale' => 'no',
			'border_pattern' => '',
			'border_pattern_image' => '',
			'border_pattern_width' => '',
