...
// backward compatibility
	if ( vc_has_class( 'prettyphoto', $el_class ) || vc_has_class( 'lightbox_single_portfolio', $el_class )) {
		$onclick = 'link_image';
	}
...
// backward compatibility
	...
	$hover_animation_html = '';
	$hover_animation_class = '';
	$data_animated_border_color = '';
	$border_offset_style = '';

	if ($hover_animation == 'yes') {
		$hover_animation_html = '<span class="shader" ' . ($overlay_image_color ? "style=background-color:$overlay_image_color" : "") . '>';

		if ($img_link_target == "open_prettyphoto") {
			$hover_animation_html .= '<span class="icon_holder"><span aria-hidden="true" class="icon_search"></span></span>';
		}

		$hover_animation_html .= '</span>';

		if ($show_border == 'yes' && $border_hover_animation != '') {

			if ($animated_border_color != '') {
				$data_animated_border_color = 'data-hover-border-color=' . $animated_border_color;
			}

			if ($inner_border_offset != '') {
				$height = 100 - (int)$inner_border_offset * 2;
				$border_offset_style = 'top:' . $inner_border_offset . '%; height:' . $height . '%;';
			}
			if ($inner_border_lr_offset != '') {
				$width = 100 - (int)$inner_border_lr_offset * 2;
				$border_offset_style .= 'left:' . $inner_border_lr_offset . '%;width:' . $width . '%;';
			}

			$hover_animation_html .= '<div class="border_box ' . $border_hover_animation . '" ' . edgt_get_inline_style($border_offset_style) . ' ' . $data_animated_border_color . '>';

			if ($border_hover_animation !== 'edgt_box_scale_line') {
				$hover_animation_html .= '<div class="border1"></div><div class="border2"></div><div class="border3"></div><div class="border4"></div>';
			}

			$hover_animation_html .= '</div>';
		}
	} else {
		$hover_animation_class = 'no_animation';
	}

	$html = ( 'vc_box_shadow_3d' === $style ) ? '<span class="vc_box_shadow_3d_wrap">' . $img['thumbnail'] . '</span>' : $img['thumbnail'];
	$html = '<div class="vc_single_image-wrapper '. $hover_animation_class. ' ' . $style . ' ' . $border_color . '">' . $hover_animation_html . $html . '</div>';
...
';
	...
	extract(shortcode_atts(array(
		...
		'overlay_image_color' => '',
		'show_border' => 'no',
		'border_hover_animation' => '',
		'inner_border_offset' => '',
		'inner_border_lr_offset' => '',
		'alignment' => 'left',
		'el_class' => '',
		'css_animation' => '',
		'style' => '',
	...
	$border_hover_animation = esc_attr($border_hover_animation);
	$overlay_image_color = esc_attr($overlay_image_color);
...
//to disable relative links uncomment this..
	...
	if ($hover_animation == 'yes') {
		...
		if ($img_link_target == "open_prettyphoto") {
			$hover_animation_html .= '<span class="icon_holder"><span aria-hidden="true" class="icon_search"></span></span>';
			$hover_animation_class .= ' prettyphoto';
		}
	}
	...
	if ($img_link_target !== "open_prettyphoto") {
		$image_string = !empty($link_to) ? '<a' . $a_class . ' href="' . $link_to . '"' . ' target="' . $img_link_target . '"' . '><div class="vc_single_image-wrapper ' . $hover_animation_class . ' ' . $style . ' ' . $border_color . '">' . $hover_animation_html . $img_output . '</div></a>' : '<div class="vc_single_image-wrapper ' . $hover_animation_class . ' ' . $style . ' ' . $border_color . '">' . $hover_animation_html . $img_output . '</div>';
	} else {
		$rel_number = rand();
		$rel = 'data-rel="prettyPhoto[rel-' . $rel_number . ']"';
		$image_string = '<div class="prettyphoto_container"><a' . $a_class . ' href="' . $link_to . '"  ' . $rel . '><div class="vc_single_image-wrapper ' . $hover_animation_class . ' ' . $style . ' ' . $border_color . '">' . $hover_animation_html . $img_output . '</div></a></div>';
	}
