...
$pattern_image = esc_attr($pattern_image);
...
elseif (strpos($style, 'icons') !== false) {
    ...
    foreach ($tab_titles as $tab) {
        ...
        if (isset($tab_matches[1][0])) {
            $tabs_nav .= '<li><a href="#tab-' . (isset($tab_matches[3][0]) ? $tab_matches[3][0] : sanitize_title($tab_matches[1][0]) ) . '"><span>' . $tab_matches[1][0] . '</span>';
			if($pattern_stripe == 'yes') {
				$tabs_nav_pattern_style = '';
				if($pattern_image != '') {						
					if (is_numeric($pattern_image)) {
						$pattern_image_src = wp_get_attachment_url($pattern_image);
					} else {
						$pattern_image_src = $pattern_image;
					}
					$tabs_nav_pattern_style .= 'style="background-image:url(' . $pattern_image_src . ');"';
				}
				$tabs_nav .= "<div class='tabs_nav_pattern' $tabs_nav_pattern_style></div>";
			}
			$tabs_nav .= '</a></li>';
        }
    }
...
$output .= "\n\t\t" . '<div class="edgt_tabs ' . $style_class . '" '.$data_attr.'>';
